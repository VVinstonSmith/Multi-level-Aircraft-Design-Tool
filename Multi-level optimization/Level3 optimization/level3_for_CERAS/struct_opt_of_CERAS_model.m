...
%% flight condition
RHO_air1 = flight_condition_paras(1);
RHO_air2 = flight_condition_paras(2);
Ma1 = flight_condition_paras(3);
Ma2 = flight_condition_paras(4);
speed_of_sound1 = flight_condition_paras(5);
speed_of_sound2 = flight_condition_paras(6);
alpha_25g = flight_condition_paras(7);
%% 机翼几何参数
C_root_wing = wing_geometry_paras(1);
taper_inner_wing = wing_geometry_paras(2);
taper_outer_wing = wing_geometry_paras(3);
span_eta = wing_geometry_paras(4);
half_span_wing = wing_geometry_paras(5);
sweep_wing_inner = wing_geometry_paras(6);
sweep_wing_outer = wing_geometry_paras(7);
body_width_wing = wing_geometry_paras(8);
X25_root_wing = wing_geometry_paras(9);
height_wing = wing_geometry_paras(10);
dihedral_wing = wing_geometry_paras(11);
n_str_wing = wing_geometry_paras(12); %桁条数量
% root
Cf_root = wing_geometry_paras(13);% 前梁相对弦长
Cr_root = wing_geometry_paras(14);% 后梁相对弦长
% kink
Cf_kink = wing_geometry_paras(15);% 前梁相对弦长
Cr_kink = wing_geometry_paras(16);% 后梁相对弦长
% tip
Cf_tip = wing_geometry_paras(17);% 前梁相对弦长
Cr_tip = wing_geometry_paras(18);% 后梁相对弦长
%
%% 平尾几何参数
C_root_htail = htail_geometry_paras(1);
taper_htail = htail_geometry_paras(2);
half_span_htail = htail_geometry_paras(3);
sweep_htail = htail_geometry_paras(4);
body_width_htail = htail_geometry_paras(5);
X25_root_htail = htail_geometry_paras(6);
height_htail = htail_geometry_paras(7);
dihedral_htail = htail_geometry_paras(8);
n_str_htail = 5;
% 根部
Cf_htail_root = htail_geometry_paras(10);% 前梁相对弦长
Cr_htail_root = htail_geometry_paras(11);% 后梁相对弦长
Cs_htail_root = linspace(Cf_htail_root, Cr_htail_root, 7);% 桁条相对弦长
% 翼梢
Cf_htail_tip = htail_geometry_paras(12);% 前梁相对弦长
Cr_htail_tip = htail_geometry_paras(13);% 后梁相对弦长
Cs_htail_tip = linspace(Cf_htail_tip, Cr_htail_tip, 7);% 桁条相对弦长

%% 垂尾几何参数
C_root_vtail = C_root_htail;
taper_vtail = taper_htail;
half_span_vtail = half_span_htail;
sweep_vtail = sweep_htail;
X25_root_vtail = X25_root_htail;
height_vtail = height_htail;
% 根部
...
%% 操纵面位置
%aileron
aileron_x = control_surface_paras(1:2);% 内侧和外侧的相对弦长
aileron_y = control_surface_paras(3:4);% 内侧和外侧的相对展长
elevator_x = control_surface_paras(5:6);% 内侧和外侧的相对弦长
rudder_x = [0.7, 0.7];% 内侧和外侧的相对弦长
%% 机身
Xf_start = fuselage_paras(1);% 机头位置
Xf_end = fuselage_paras(2);% 机尾位置
Zf = fuselage_paras(3);
%% independent weight
mass_body = [weight_paras(1)];
cg_body = [weight_paras(2), 0.,0.];
fuel_mass = weight_paras(3) + weight_paras(4);
cg_whole_est = weight_paras(5);

Xf_seq = [Xf_start, 13.43, ...
    [16.0 : 0.05 : 17.0], ...
    32.63, 33.5, 35.18, Xf_end];
Zf_seq = [Zf, Zf, ...
    Zf*ones(1,21),...
    height_htail, height_htail, height_htail, height_htail];
